...
keysData = ns.recv(nKeys * 16, socket.MSG_WAITALL)
for i in range(nKeys):
  key_dst_ip = int.from_bytes(keysData[i*16:i*12+4], "big", signed=False)
  key_pkt_size = int.from_bytes(keysData[i*16+4:i*12+8], "big", signed=False)
  vaddr = int.from_bytes(keysData[i*16+8:i*12+16], "big", signed=False)
  prog.pipe.SwitchEgress.tab_addr.add_with_addr_tab_hit(dst_ip=key_dst_ip, pkt_size=key_pkt_size, vaddr=vaddr)
...
prog.pipe.SwitchEgress.reg_cm_x.clear()
prog.pipe.SwitchEgress.reg_cm_y.clear()
