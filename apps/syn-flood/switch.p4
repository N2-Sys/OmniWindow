...
control SwitchEgress(
  ...
  @stage(0) {
    if (hdr.eg_op.op[3:2] == 0 && hdr.tcp.isValid()) {
      if (hdr.tcp.flags == 17) {
        eg_md.key.setValid();
        eg_md.key.dst_ip = hdr.ipv4.src_addr;
      } else if (hdr.tcp.flags == 2 || hdr.tcp.flags == 16) {
        eg_md.key.setValid();
  ...
  @stage(2) {
    if (hdr.eg_op.op[3:2] == 0b11) { // reset
      ...
      filt_any = 0;
    } else if (filt1 == 1 || filt2 == 1 || filt3 == 1) { // normal
  ...
  @stage(5) {
    cm_h_x = hash_cm2_x.get(eg_md.key);
  }
  ...
  @stage(8) {
    if (hdr.eg_op.op[3:2] == 0b11 && reset_idx != NDATA) { // reset
      reset_cm3_x.execute(epoch[0:0] ++ reset_idx[15:0]);
    } else if (filt_any == 1) { // normal
      if (filt3 == 1)
        upd_cm3_x.execute(epoch[0:0] ++ cm_h_x);
    } else if (hdr.eg_op.op[3:2] == 0b10 && eg_md.key.isValid()) { // col && has key to col
      cm3_v = get_cm3_x.execute(epoch[0:0] ++ cm_h_x);
    }
  }
  ...
  @stage(9) {
    hdr.val = {cm1_v[7:0], cm1_v[15:8], cm2_v[7:0], cm2_v[15:8], cm3_v[7:0], cm3_v[15:8]};
    if (hdr.dc_resp.isValid()) {
