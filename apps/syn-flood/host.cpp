...
class Server {
  ...
  static constexpr uint32_t MSG_BUF_SIZE = 3 << 23;
...
#ifdef SLIDING
      epochData[curEpochPos][pos] = {msg.val1, msg.val2, msg.val3, 0};
#else
      epochData[pos] = {msg.val1, msg.val2, msg.val3, 0};
#endif
    }
  }
...
public:
  ...
  void switchWin(uint16_t nReq = 1) {
    ...
    dc.sendReq({DCReqOp::RST_START, {}});
    for (uint16_t i = 1; i < nReq; i++)
      dc.sendReq({DCReqOp::RST, {}});
    ...
    for (uint32_t i = 0; i < N; i++)
      if ((valid[i / 16] >> (i % 16) & 1) && (int)mergedData[i].v1 + mergedData[i].v2 - mergedData[i].v3 >= threshold) {
    ...
    for (uint32_t i = 0; i < N; i += 512 / 64) {
      ...
      x = _mm512_sub_epi16(x, y);
      _mm512_store_epi32(mergedData + i, x);
    }
    clock_gettime(CLOCK_REALTIME, &tEndSub);

    for (uint32_t i = 0; i < N; i++)
      if ((valid[i / 16] >> (i % 16) & 1) && mergedData[i].v1 == 0 && mergedData[i].v2 == 0 && mergedData[i].v3 == 0) {
        delKeys[nDel++] = keys[i];
        valid[i / 16] &= ~(1 << (i % 16));
      }
...
#error "unimplemented"
    ...
    for (uint32_t i = 0; i < nKeys; i++)
      if ((int)mergedData[i].v1 + mergedData[i].v2 - mergedData[i].v3 >= threshold) {
        abnormalKeys[nAbnormal++] = keys[i];
      }
      clock_gettime(CLOCK_REALTIME, &tEndMerge);
    }
