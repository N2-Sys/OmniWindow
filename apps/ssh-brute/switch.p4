...
parser SwitchEgressParser(
  ...
  state parse_ipv4 {
    ...
    eg_md.key.pkt_size = (bit<32>)hdr.ipv4.total_len;

    eg_md.dkey.setValid();
    eg_md.dkey.dst_ip = hdr.ipv4.dst_addr;
    eg_md.dkey.src_ip = hdr.ipv4.src_addr;
    eg_md.dkey.pkt_size = (bit<32>)hdr.ipv4.total_len;
...
control SwitchEgress(
  ...
  Register<bit<32>, bit<17>>(1 << 17, 0) reg_pkt_size;
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_pkt_size) store_pkt_size = {
    void apply(inout bit<32> val) {
      val = eg_md.key.pkt_size;
    }
  };
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_pkt_size) get_pkt_size = {
    void apply(inout bit<32> val, out bit<32> res) {
      res = val;
    }
  };
  ...
  @stage(4) {
    if (distinct && (bfv_x == 0 || bfv_y == 0)) { // store new key
  ...
  @stage(6) {
    if (store_new && key_idx != NKEYS) { // store new key
      store_pkt_size.execute(epoch[0:0] ++ key_idx[15:0]);
    } else if (hdr.eg_op.op[3:1] == 0b100 && key_idx != NKEYS) { // col key
      eg_md.key.pkt_size = get_pkt_size.execute(epoch[0:0] ++ key_idx[15:0]);
    }
  }
  ...
  @stage(7) {
    keyv = eg_md.key.dst_ip ++ eg_md.key.pkt_size;
  ...
  @stage(11) {
    ...
    if (hdr.eg_op.op[3:1] == 0b100) { // col keys
      ...
      eg_md.mirror_hdr.key_pkt_size = eg_md.key.pkt_size;
    }
