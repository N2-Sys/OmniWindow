...
// #define SLIDING
// #define PROC_OVERLAP
#define EPOCH_DATA_SIZE (1 << 20)

#define NDATA 65536
#define NKEYS 16384

#define MSG_LEN 16
#define MSG_BUF_SIZE (4 << 23)
#define MSG_BATCH_SIZE (4 << 13)
...
parser SwitchEgressParser(
  ...
  state parse_ipv4 {
    ...
    eg_md.dkey.setValid();
    eg_md.dkey.dst_ip = hdr.ipv4.dst_addr;
    eg_md.dkey.src_ip = hdr.ipv4.src_addr;
  ...
  state parse_udp {
    pkt.extract(hdr.udp);
    eg_md.dkey.src_port = hdr.udp.src_port;
    transition select (hdr.udp.dst_port, hdr.udp.src_port) {
  ...
  state parse_tcp {
    pkt.extract(hdr.tcp);
    eg_md.dkey.src_port = hdr.tcp.src_port;
    transition accept;
  }
...
control SwitchEgress(
  ...
  BLOOM_FILTER_16_DUP(dbf_x, 0x8005)
  BLOOM_FILTER_16_DUP(dbf_y, 0x0589)
  ...
  COUNT_MIN_16_DUP(cm1_x, 0x8005)
  // COUNT_MIN_16_DUP(cm1_y, 0x0589)

  Register<bit<32>, bit<17>>(1 << 17, 0) reg_cm2_x;
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_cm2_x) upd_cm2_x = {
    void apply(inout bit<32> val, out bit<32> res) {
      res = val;
      val = val + (bit<32>)hdr.ipv4.total_len;
    }
  };
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_cm2_x) reset_cm2_x = {
    void apply(inout bit<32> val) {
      val = 0;
    }
  };
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_cm2_x) get_cm2_x = {
    void apply(inout bit<32> val, out bit<32> res) {
      res = val;
    }
  };

  // COUNT_MIN_16_DUP(cm2_x, 0x8005)
  // COUNT_MIN_16_DUP(cm2_y, 0x0589)
  ...
  @stage(0) {
    dbf_h_x = hash_dbf_x.get(eg_md.dkey);
    dbf_h_y = hash_dbf_y.get(eg_md.dkey);
    ...
    bit<1> dbfv_x;
    bit<1> dbfv_y;
    bool filt = hdr.eg_op.op[3:2] == 0 && hdr.tcp.isValid();

  @stage(1) {
    if (hdr.eg_op.op[3:2] == 0b11 && hdr.eg_op.op[8:8] == 1) { // reset
      reset_dbf_x.execute(epoch[0:0] ++ reset_idx[15:0]);
      reset_dbf_y.execute(epoch[0:0] ++ reset_idx[15:0]);
    } else if (filt) { // normal
      dbfv_x = upd_dbf_x.execute(epoch[0:0] ++ dbf_h_x);
      dbfv_y = upd_dbf_y.execute(epoch[0:0] ++ dbf_h_y);
    }
  }
  ...
  @stage(2) {
    ...
    bool distinct = filt && (dbfv_x == 0 || dbfv_y == 0);
  ...
  @stage(6) {
    cm_h_x = hash_cm1_x.get(eg_md.key);
  }

  @stage(7) {
    if (hdr.eg_op.op[3:2] == 0b11 && reset_idx != NDATA) { // reset
      reset_cm1_x.execute(epoch[0:0] ++ reset_idx[15:0]);
    } else if (distinct) { // normal
      upd_cm1_x.execute(epoch[0:0] ++ cm_h_x);
    } else if (hdr.eg_op.op[3:2] == 0b10 && eg_md.key.isValid()) { // col && has key to col
      cm1_v = get_cm1_x.execute(epoch[0:0] ++ cm_h_x);
    }
  }

  @stage(8) {
    if (hdr.eg_op.op[3:2] == 0b11 && reset_idx != NDATA) { // reset
      reset_cm2_x.execute(epoch[0:0] ++ reset_idx[15:0]);
    } else if (filt) { // normal
      upd_cm2_x.execute(epoch[0:0] ++ cm_h_x);
    } else if (hdr.eg_op.op[3:2] == 0b10 && eg_md.key.isValid()) { // col && has key to col
      cm2_v = get_cm2_x.execute(epoch[0:0] ++ cm_h_x);
    }
  }
  ...
  // @stage(5, 6) {
    ...
    if (hdr.dc_resp.isValid()) {
      ...
      if (hdr.dc_resp.op == 0) {
        ...
        hdr.dc_ipadding.setValid();
        immdtv = hdr.dc_ipadding.v;
  ...
  // }
  @stage(9) {
    hdr.val = {cm2_v[7:0], cm2_v[15:8], cm2_v[23:16], cm2_v[31:24], cm1_v[7:0], cm1_v[15:8]};
  }

  @stage(10) {
    if (hdr.dc_resp.isValid()) {
      if (hdr.ib_immdt.isValid()) {
        hdr.ib_reth.vaddr = hdr.ib_reth.vaddr + 4;
      }
    } else if (hdr.vaddr.isValid()) {
  ...
  @stage(11) {
    ...
    if (hdr.eg_op.op[3:1] == 0b100) { // col keys
      ...
      if (key_idx == 0 || (key_idx == NKEYS && hdr.eg_op.op[0:0] == 1)) {
        eg_md.mirror_hdr.eg_op = hdr.eg_op.op[15:8] ++ 8w0b10001100;
      } else if (key_idx == NKEYS) {
        eg_md.mirror_hdr.eg_op = hdr.eg_op.op[15:8] ++ 8w0b00001100;
      } else {
        eg_md.mirror_hdr.eg_op = hdr.eg_op.op[15:8] ++ 8w0b00001000;
      }
      ...
      eg_md.mirror_session = MIRROR_SESSION_EG_DC;
      eg_md.mirror_hdr.setValid();
      eg_md.mirror_hdr.eg_op = hdr.eg_op.op[15:8] ++ 8w0b00000110;
