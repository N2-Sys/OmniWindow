...
/* Session ID */
const MirrorId_t MIRROR_EG_RESET = 2;

const bit<16> UDP_PORT_RESET = 33669;
...
parser SwitchIngressParser(
  ...
  state parse_udp {
    ...
    transition select (hdr.udp.dst_port) {
      UDP_PORT_RESET: parse_reset_req;
      default: accept;
    }
  }

  state parse_reset_req {
    pkt.extract(hdr.reset_req);
    transition accept;
  }
}
...
control SwitchIngress(
  ...
  apply {
    ...
    if (tab_reset.apply().hit) {
      if (hdr.reset_req.idx == 0)
        hdr.eg_op.op = 8w0b00000110;
      else
        hdr.eg_op.op = 8w0b00000010;
...
control SwitchEgress(
  ...
  RegisterAction<bit<16>, bit<16>, bit<16>>(reg_data_0) reset_data_0 = {
  ...
  RegisterAction<bit<16>, bit<16>, bit<16>>(reg_data_1) reset_data_1 = {
    void apply(inout bit<16> val) {
      val = 0;
    }
  };
  ...
  RegisterAction<bit<16>, bit<16>, bit<16>>(reg_data_2) reset_data_2 = {
    void apply(inout bit<16> val) {
      val = 0;
    }
  };
  ...
  RegisterAction<bit<16>, bit<16>, bit<16>>(reg_data_3) reset_data_3 = {
    void apply(inout bit<16> val) {
      val = 0;
    }
  };
  ...
  RegisterAction<bit<32>, bit<0>, bit<32>>(reg_reset_idx) start_reset = {
    void apply(inout bit<32> val) {
      val = 0;
    }
  };
  RegisterAction<bit<32>, bit<0>, bit<32>>(reg_reset_idx) get_next_reset_idx = {
  ...
  apply {
    ...
    if (hdr.eg_op.op[2:2] == 1) {
      reset_idx = 0;
      start_reset.execute(0);
    } else if (hdr.eg_op.op[1:1] == 1) {
    ...
    } else if (hdr.eg_op.op[1:1] == 1 && reset_idx != N) {
      reset_data_0.execute(reset_idx[15:0]);
#if CM_NUM > 1
      reset_data_1.execute(reset_idx[15:0]);
#endif
#if CM_NUM > 2
      reset_data_2.execute(reset_idx[15:0]);
#endif
#if CM_NUM > 3
      reset_data_3.execute(reset_idx[15:0]);
#endif
    }

    if (hdr.eg_op.op[1:1] == 1 && reset_idx != N - 1) {
      eg_dprsr_md.drop_ctl = 0x1;
    }

    eg_dprsr_md.mirror_type = 0;
    if (hdr.eg_op.op[1:1] == 1 && reset_idx != N) {
      eg_dprsr_md.mirror_type = EG_PORT_MIRROR;
      eg_md.mirror_session = MIRROR_EG_RESET;
      eg_md.mirror_eg_op.setValid();
      eg_md.mirror_eg_op.op = 8w0b00000010;
    }
