...
parser SwitchEgressParser(
  ...
  state parse_ipv4 {
    ...
    eg_md.key.setValid();
    eg_md.key.src_ip = hdr.ipv4.src_addr;
  ...
  state parse_udp {
    pkt.extract(hdr.udp);
    eg_md.dkey.dst_port = hdr.udp.dst_port;
    transition select (hdr.udp.dst_port, hdr.udp.src_port) {
  ...
  state parse_tcp {
    pkt.extract(hdr.tcp);
    eg_md.dkey.dst_port = hdr.tcp.dst_port;
    transition accept;
  }
...
control SwitchEgress(
  ...
  Register<bit<32>, bit<17>>(1 << 17, 0) reg_src_ip;
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_src_ip) store_src_ip = {
    void apply(inout bit<32> val) {
      val = eg_md.key.src_ip;
    }
  };
  RegisterAction<bit<32>, bit<17>, bit<32>>(reg_src_ip) get_src_ip = {
    void apply(inout bit<32> val, out bit<32> res) {
      res = val;
    }
  };
  ...
  @stage(5) {
    if (store_new && key_idx != NKEYS) { // store new key
      store_src_ip.execute(epoch[0:0] ++ key_idx[15:0]);
    } else if (hdr.eg_op.op[3:1] == 0b100 && key_idx != NKEYS) { // col key
      eg_md.key.setValid();
      eg_md.key.src_ip = get_src_ip.execute(epoch[0:0] ++ key_idx[15:0]);
    }
  }
  ...
  @stage(9) {
    ...
    if (hdr.eg_op.op[3:1] == 0b100) { // col keys
      ...
      eg_md.mirror_hdr.key_src_ip = eg_md.key.src_ip;
    }
