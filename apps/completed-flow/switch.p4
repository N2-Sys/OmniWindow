...
#define MSG_LEN 12
#define MSG_BUF_SIZE (3 << 23)
#define MSG_BATCH_SIZE (3 << 13)
...
control SwitchEgress(
  ...
  @stage(0) {
    ...
    if (hdr.eg_op.op[3:2] == 0 && hdr.tcp.isValid()) {
      if (hdr.tcp.flags == 0x2)
        filt1 = 1;
      else
        filt1 = 0;
      if (hdr.tcp.flags[0:0] == 1)
        filt2 = 1;
      else
        filt2 = 0;
    } else {
  ...
  @stage(1) {
    if (hdr.eg_op.op[3:2] == 0b11) { // reset
      ...
      filt_any = 0;
    } else if (filt1 == 1 || filt2 == 1) { // normal
  ...
  @stage(2) {
    if (filt_any == 1 && (bfv_x == 0 || bfv_y == 0)) { // store new key
  ...
  @stage(4) {
    ...
    cm_h_y = hash_cm1_y.get(eg_md.key);
  }
  ...
  @stage(7) {
    if (hdr.eg_op.op[3:2] == 0b11 && reset_idx != NDATA) { // reset
      reset_cm1_y.execute(epoch[0:0] ++ reset_idx[15:0]);
    } else if (filt_any == 1) { // normal
      if (filt1 == 1)
        upd_cm1_y.execute(cm_idx_y);
    } else if (hdr.eg_op.op[3:2] == 0b10 && eg_md.key.isValid()) { // col && has key to col
      cm1_v = min(cm1_v, get_cm1_y.execute(cm_idx_y));
    }
  }

  @stage(8) {
    if (hdr.eg_op.op[3:2] == 0b11 && reset_idx != NDATA) { // reset
      reset_cm2_y.execute(epoch[0:0] ++ reset_idx[15:0]);
    } else if (filt_any == 1) { // normal
      if (filt2 == 1)
        upd_cm2_y.execute(cm_idx_y);
    } else if (hdr.eg_op.op[3:2] == 0b10 && eg_md.key.isValid()) { // col && has key to col
      cm2_v = min(cm2_v, get_cm2_y.execute(cm_idx_y));
    }
  }
  ...
  @stage(9) {
    hdr.val = {cm1_v[7:0], cm1_v[15:8], cm2_v[7:0], cm2_v[15:8]};
    if (hdr.dc_resp.isValid()) {
