...
#ifdef SLIDING
      epochData[curEpochPos][pos] = {msg.val1, msg.val2};
#else
      epochData[pos] = {msg.val1, msg.val2};
#endif
    }
  }
...
public:
  ...
  void switchWin(uint16_t nReq = 1) {
    ...
    __m512i vML = _mm512_set1_epi32(0xFFFF);
    for (uint32_t i = 0; i < N; i += 512 / 32) {
      __m512i x, x1, x2, vKeys;
      __mmask16 mAbnormal;
    ...
    for (uint32_t i = 0; i < N; i += 512 / 32) {
      ...
      _mm512_mask_compressstoreu_epi32(delKeys + nDel, mDel, vKeys);
      nDel += _popcnt32(mDel);
      valid[i / 16] ^= mDel;
    }
...
#error "unimplemented"
    ...
    if (curEpoch != 0) {
      ...
      __m512i vThres = _mm512_set1_epi32(threshold);
      __m512i vML = _mm512_set1_epi32(0xFFFF);
      for (uint32_t i = 0; i < nKeysN; i += 512 / 32) {
        __m512i x, x1, x2, vKeys;
        __mmask16 mAbnormal;
        vKeys = _mm512_load_epi32(keys + i);
        x = _mm512_load_epi32(mergedData + i);
