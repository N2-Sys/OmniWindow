...
class Server {
  ...
  static constexpr uint32_t N = 1 << 17;
#ifdef SLIDING
...
#ifdef SLIDING
      epochData[curEpochPos][pos] = {msg.val2, msg.val1, 0};
#else
      epochData[pos] = {msg.val2, msg.val1, 0};
#endif
    }
  }
...
public:
  Server(uint32_t nEpoch_, uint32_t threshold1_, double threshold2_, int dcPortId,
         const char *dcSrcIp, const char *rdmaDevName, NetworkStream &ns) :
      mem{},
#ifdef SLIDING
      valid{}, pin{},
#endif
      curEpoch(0), nEpoch(nEpoch_), threshold1(threshold1_), threshold2(threshold2_),
#ifndef SLIDING
      nKeys(0),
#endif
      msgHead(0), msgTail(0),
  ...
  void switchWin(uint16_t nReq = 1) {
    ...
    for (uint32_t i = 0; i < N; i++)
      if ((valid[i / 16] >> (i % 16) & 1) && mergedData[i].v2 >= threshold1 && (double)mergedData[i].v1 / mergedData[i].v2 >= threshold2) {
        abnormalKeys[nAbnormal++] = keys[i];
      }
    ...
    for (uint32_t i = 0; i < N; i++)
      if ((valid[i / 16] >> (i % 16) & 1) && mergedData[i].v1 == 0 && mergedData[i].v2 == 0) {
        delKeys[nDel++] = keys[i];
        valid[i / 16] &= ~(1 << (i % 16));
      }
...
#error "unimplemented"
    ...
    if (curEpoch != 0) {
#endif
      for (uint32_t i = 0; i < nKeys; i += 512 / 64) {
        __m512i x, y;
        x = _mm512_load_epi32(mergedData + i);
      ...
      for (uint32_t i = 0; i < N; i++)
        if (mergedData[i].v2 >= threshold1 && (double)mergedData[i].v1 / mergedData[i].v2 >= threshold2) {
          abnormalKeys[nAbnormal++] = keys[i];
        }
      clock_gettime(CLOCK_REALTIME, &tEndMerge);
    }
...
//   }
  ...
  void insKeys(const char *input_path, NetworkStream &ns) {
    ...
    uint32_t nKeys = 0;
#endif
    while ((rc = fscanf(fp_in, "%x%*u", &key)) == 2) {
      key = htonl(key);
...
int main(int argc, char **argv) {
  ...
  uint32_t nSubWins = 5;
  uint32_t threshold1 = 7500;
  double threshold2 = 0.004;
  char *hostIp = argv[2];
  char *rdmaDev = argv[3];
  ...
  Server *s = new Server(nSubWins, threshold1, threshold2, dcPortId, hostIp, rdmaDev, ns);
